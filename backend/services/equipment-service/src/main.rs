...
use repository::*;
use service::EquipmentService;
use api::{QueryRoot, MutationRoot};
use config::Config;

type EquipmentSchema = Schema<QueryRoot, MutationRoot, EmptySubscription>;

async fn graphql(
    schema: web::Data<EquipmentSchema>,
    req: GraphQLRequest,
) -> GraphQLResponse {
...
async fn main() -> std::io::Result<()> {
    ...
    tracing::info!("Starting Equipment Service...");
    ...
    // Create repositories
    let equipment_repo = EquipmentRepository::new(pool.clone());
    let maintenance_repo = EquipmentMaintenanceRepository::new(pool.clone());
    let calibration_repo = EquipmentCalibrationRepository::new(pool.clone());
    let test_assignment_repo = EquipmentTestAssignmentRepository::new(pool.clone());
    let performance_log_repo = EquipmentPerformanceLogRepository::new(pool.clone());
    let alert_repo = EquipmentAlertRepository::new(pool.clone());
    ...
    tracing::info!("  Queries: equipment, equipmentByCode, equipmentList, maintenance, maintenanceList, calibration, calibrationList, testAssignments, performanceLogs, alert, alertList");
    tracing::info!("  Mutations: createEquipment, updateEquipment, updateEquipmentStatus, deleteEquipment, scheduleMaintenance, completeMaintenance, cancelMaintenance, recordCalibration, assignTest, unassignTest, logPerformance, acknowledgeAlert, resolveAlert, incrementTestCounter");
