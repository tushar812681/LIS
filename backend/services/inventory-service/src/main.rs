...
type InventorySchema = Schema<QueryRoot, MutationRoot, EmptySubscription>;

async fn graphql(
    schema: web::Data<InventorySchema>,
    req: GraphQLRequest,
) -> GraphQLResponse {
...
async fn main() -> std::io::Result<()> {
    ...
    let item_repo = InventoryItemRepository::new(pool.clone());
    let batch_repo = StockBatchRepository::new(pool.clone());
    let movement_repo = StockMovementRepository::new(pool.clone());
    let po_repo = PurchaseOrderRepository::new(pool.clone());
    let alert_repo = StockAlertRepository::new(pool.clone());
    ...
    tracing::info!("  Queries: vendor, vendors, inventoryItem, inventoryItems, lowStockItems, stockBatch, itemBatches, expiringBatches, stockMovements, purchaseOrder, purchaseOrderItems, purchaseOrders, stockAlerts");
    tracing::info!("  Mutations: createVendor, createInventoryItem, createStockBatch, recordStockMovement, createPurchaseOrder, approvePurchaseOrder, receivePurchaseOrder, resolveStockAlert");
